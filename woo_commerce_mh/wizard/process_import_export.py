...
class WooProcessImportExport(models.TransientModel):
    ...
    def execute(self):
        ...
        """
        queues = False
        actions = {"import_customer": ["woo_commerce_mh.woo_customer_data_queue_ept_action",
                                       "woo_commerce_mh.woo_customer_data_data_queue_ept_form_view"],
                   "import_product": ["woo_commerce_mh.action_woo_product_data_queue_ept",
                                      "woo_commerce_mh.woo_product_data_queue_form_view_ept"],
                   "import_unshipped_orders": ["woo_commerce_mh.action_woo_unshipped_order_queue_ept",
                                               "woo_commerce_mh.view_woo_order_data_queue_ept_form"],
                   "import_completed_orders": ["woo_commerce_mh.action_woo_shipped_order_queue_ept",
                                               "woo_commerce_mh.view_woo_order_data_queue_ept_form"],
                   "import_coupon": ["woo_commerce_mh.action_woo_coupon_data_queue_ept",
                                     "woo_commerce_mh.view_woo_coupon_data_queue_ept_form"]}
    ...
    def preapre_template_vals_from_csv_data(self, record, instance_id, product_type):
        ...
        if self.env["ir.config_parameter"].sudo().get_param("woo_commerce_mh.set_sales_description"):
    ...
    def woo_check_running_schedulers(self, cron_xml_id):
        ...
        """
        try:
            cron_id = self.env.ref('woo_commerce_mh.%s%d' % (cron_xml_id, self.woo_instance_id.id))
        except Exception as error:
            return
